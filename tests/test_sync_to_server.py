...
def test_run_locked_takes_and_releases_the_pipeline_lock(monkeypatch):
    seen = {}
    monkeypatch.setattr(s2s, "run", lambda dry_run=False: (seen.update(held=pl.LOCK_PATH.exists())) or s2s.ShelfUploadResult(True, True, True, "ok"))
    s2s.run_locked()
...
def test_run_locked_publishes_result(monkeypatch, isolated_env):
    monkeypatch.setattr(s2s, "run", lambda dry_run=False: s2s.ShelfUploadResult(False, False, None, "not configured"))
    s2s.run_locked()
...
def test_main_exits_zero_on_success(monkeypatch, capsys):
    ...
    monkeypatch.setattr(s2s, "run_locked", lambda dry_run=False: s2s.ShelfUploadResult(True, True, True, "pushed"))
    assert s2s.main() == 0
...
def test_main_exits_nonzero_on_not_configured(monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["sync_to_server.py"])
    monkeypatch.setattr(s2s, "run_locked", lambda dry_run=False: s2s.ShelfUploadResult(False, False, None, "not configured"))
    assert s2s.main() == 1
...
def test_main_exits_nonzero_when_locked(monkeypatch):
    ...
    def raise_locked(dry_run=False):
