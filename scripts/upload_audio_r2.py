...
def record_payload(files: Dict[str, dict]) -> dict:
    """The manifest document, as one dict. THE canonical shape, in one place.

    ⚠️ Split out of `write_record` for audio phase 1 (2026-08-18) so
    `scripts/publish_audio_manifest.py` can build the payload the Worker reads
    WITHOUT going through a file on disk — which it must, because the honest
    answer before anyone has requested a book is an EMPTY manifest, and there
    is no local file to read in that case. Two implementations of this shape
    would be two answers to "what is streamable"; the Worker's index would
    silently follow whichever one published last.
    """
    return {
        "_comment": (
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # ⚠️ An upload nobody published is invisible. The Worker reads the manifest
    # out of a PRIVATE R2 bucket, never out of site/audio_manifest.json — so a
    # hand-run upload that skipped this leaves a 600 MB object in R2 that the
    # player still answers "not streamable yet" for. Soft, and a no-op when the
    # record did not change. (audio phase 1, 2026-08-18)
    try:
